...
echo "$($PSQL "SELECT AVG(winner_goals + opponent_goals) AS rounded_number_of_goals FROM games")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL "SELECT MAX(winner_goals) AS max_goals_in_a_single_game FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) AS number_of_games_where_winning_team_more_than_two_goals FROM games WHERE winner_goals > 2")"
...
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN games on teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name")"
